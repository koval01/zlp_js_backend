const request = require("request"), {checkTelegramAuthorization} = require("../helpers/telegram/base"),
    re_check = (req, resp, next) => {
        request(
            {
        ...
        try { auth_data = JSON.parse(
            Buffer.from(auth_data, 'base64').toString())
        } catch (_) {
